...
namespace cage
{
	...
	namespace privat
	{
		template<class T> struct GeometryOrder { static constexpr int order = 0; };
		...
		template<class A, class B> static constexpr bool geometrySwapParameters = GeometryOrder<A>::order > GeometryOrder<B>::order && GeometryOrder<B>::order > 0;
	}
