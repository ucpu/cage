...
namespace cage
{
	...
	namespace detail
	{
		...
		CAGE_CORE_API bool readLine(string &output, const char *&buffer, uintPtr &size, bool lfOnly);
	}
	...
	CAGE_CORE_API Holder<LineReader> newLineReader(const char *buffer, uintPtr size); // the buffer must outlive the reader
	CAGE_CORE_API Holder<LineReader> newLineReader(const MemoryBuffer &buffer); // the buffer must outlive the reader
