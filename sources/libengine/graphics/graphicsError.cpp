	...
	void checkGlError()
	{
		...
		switch (err)
		{
			...
			case GL_INVALID_ENUM:
				CAGE_THROW_ERROR(GraphicsError, "gl_invalid_enum", err);
				break;
			case GL_INVALID_VALUE:
				CAGE_THROW_ERROR(GraphicsError, "gl_invalid_value", err);
				break;
			case GL_INVALID_OPERATION:
				CAGE_THROW_ERROR(GraphicsError, "gl_invalid_operation", err);
				break;
			case GL_INVALID_FRAMEBUFFER_OPERATION:
				CAGE_THROW_ERROR(GraphicsError, "gl_invalid_framebuffer_operation", err);
				break;
			case GL_OUT_OF_MEMORY:
				CAGE_THROW_ERROR(GraphicsError, "gl_out_of_memory", err);
				break;
			default:
				CAGE_THROW_CRITICAL(GraphicsError, "gl_unknown_error", err);
				break;
		}
	}
...
#define checkExtension(NAME) \
	if (!NAME) \
	{ \
		CAGE_THROW_ERROR(Exception, "missing required OpenGL extension: " #NAME); \
	}
...
#undef checkExtension
	...
	#define GCHL_END " 2>nul 1>nul; "
				system(GCHL_START "\"%APPDATA%\\AMD\\GLCache\"" GCHL_END GCHL_START "\"%APPDATA%\\NVIDIA\\GLCache\"" GCHL_END GCHL_START "\"%LOCALAPPDATA%\\AMD\\GLCache\"" GCHL_END GCHL_START "\"%LOCALAPPDATA%\\NVIDIA\\GLCache\"" GCHL_END);
	#undef GCHL_START
	#undef GCHL_END
			}
			catch (...)
