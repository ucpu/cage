	...
	namespace
	{
		...
		*/
		...
		struct GraphicsPrepareImpl
		{
			...
			{
				if (!intersects(m->getBoundingBox(), ConservativeFrustum(mvp)))
					return;
			...
			void addLight(RenderPassImpl *pass, EmitLight *light)
			{
				...
				case LightTypeEnum::Spot:
					...
					if (!intersects(modelCone->getBoundingBox(), ConservativeFrustum(mvpMat)))
				...
				case LightTypeEnum::Point:
					...
					if (!intersects(modelSphere->getBoundingBox(), ConservativeFrustum(mvpMat)))
						return; // this light's volume is outside view frustum
					break;
				}
